...
public:
	...
	void atakuotiCPU(int &eil, int &stul, int pataike) // Algoritmas pasirenka kokias kordinates atakuoti
	{
		int stop = false;
		int c = 0;
		int temp = 0;
		...
		while (!stop)
		{
			...
			if (pataike == 1 && kryptis == 0)
			{
				kryptis = rand() % 4 + 1;
				CPUkryptis.push_back(kryptis);
				teil = CPUeil[(CPUeil.size() - 1)];
				tstul = CPUstul[(CPUstul.size() - 1)];
				tteil = &teil;
				ttstul = &tstul;
			}
			if (kryptis > 0)
			{
				...
				if (pataike > 1)
				{
					do
					{
						temp = 0;
						kryptis = rand() % 4 + 1;
						for (int i = 0; i < CPUkryptis.size(); i++)
						{
							if (CPUkryptis[i] == kryptis)
								temp++;
						}
						if (temp == 0)
							temp = -1;
				...
				if (pataike == 1)
				{
					if (kryptis == 1)
					{
						eil = teil + 1;
						stul = tstul;
						if (eil >= 0 && eil < 10 && stul >= 0 && stul < 10)
						{
							teil++;
						}
						else
			...
			for (int i = 1; i < CPUeil.size(); i++)
			{
				if (CPUeil.at(i - 1) == eil && CPUstul.at(i - 1) == stul)
				{
					c++;
				}
			}
			if (c == 0)
			{
				CPUeil.push_back(eil);
				CPUstul.push_back(stul);
				stop = true;
			}
			else
			{
	...
	int ArPataikeCPU(int eil, int stul) // cpu tikrina ar priesininkas pataike neisveda jokio teksto
	{
		int paskendo = 0;
		for (int i = 0; i < 10; i++)
		{
